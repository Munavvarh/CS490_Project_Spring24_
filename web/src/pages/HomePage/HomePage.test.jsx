...
describe('HomePage', () => {
  ...
  it('renders successfully', () => {
    ...
    expect(
      screen.getByText('The best code translator tool website')
    ).toBeInTheDocument() // Header subtitle renders
    ...
    ).toBeInTheDocument() // Section content renders
    expect(
      screen.getByText(
    ...
    expect(
      screen.getByText('© 2024 SyntaxSwitch. All rights reserved.')
    ).toBeInTheDocument() // Footer content renders
  })
  ...
  test('shows at most 5 feedbacks', async () => {
    render(<HomePage />)
    await waitFor(() => {
      expect(screen.getAllByText('Review:').length).toBeLessThanOrEqual(5)
      expect(screen.getAllByText('Rating:').length).toBeLessThanOrEqual(5)
